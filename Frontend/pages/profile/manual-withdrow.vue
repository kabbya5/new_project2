<template>
    <div>
        ...
        <div class="w-full h-full flex text-white items-center justify-center px-5 py-8">
            <div class="bg-red-900 dark:bg-green-900 shadow-lg rounded-2xl w-full md:w-[450px] px-5 pt-3 pb-5">
                ...
                <!-- Input Field -->
                <div class="mb-4">
                    <label class="block text-md font-medium text-white mb-2">
                        Number 
                    </label>
                    ...
                    <input type="text" 
                        ...
                        class="w-full px-4 py-2 border rounded-lg text-gray-800 
                            border-gray-300 dark:border-gray-600
                            bg-white focus:ring-2 focus:ring-green-500 outline-none"
                    />
                ...
                <div class="mb-4">
                    ...
                    <div>
                        ...
                        <p @click="recallBalance" class="cursor-pointer text-center mt-5"> 
                            <span class="rounded-md px-4 py-2 bg-white text-gray-700">
                ...
                <div class="mb-4" v-if="authStore.user?.turnover > 0">
                    ...
                    <div>
                        <p class="text-sm text-red-300"> Please complete the required turnover for withdrawal. </p>
                        <p class="text-sm text-red-300"> Turnover requirements <span class="uppercase"> {{ authStore.getUser()?.currency }} </span> {{ authStore.getUser()?.turnover }} </p>
                    </div>
                </div>
                ...
                <div v-else>
                    <div class="mb-4">
                        ...
                        <input type="number" v-model="amount" 
                            ...
                            class="w-full px-4 py-2 border rounded-lg text-gray-800  bg-white focus:ring-2 focus:ring-green-500 outline-none
                                border-gray-300 dark:border-gray-600"
                        />
...
<script setup lang="ts">
    ...
    const withdrow = async () => {
        ...
        try {
            ...
            const response = await useApiFetch('/manualy/withdrow/store', {
                method: 'POST',
                body: formData,
            });

            if(response.status == 1){
                alert('Transaction Record has been processing');
                router.push('/profile');
