<template>
  <div class="min-h-screen bg-red-800 dark:bg-green-800 py-4 px-4">
    <div class="max-w-md mx-auto bg-red-900 dark:bg-green-900 rounded-2xl shadow-xl px-6 pt-3 pb-6">

      <!-- Title -->
      <h1 class="text-2xl font-bold text-center mb-3 text-white">
        {{ deposite }}
      </h1>
            ...
            <div class="grid grid-cols-3 gap-5 mb-6">
                <div class="col-span-1" v-for="channel in channels" :key="channel.id">
                    ...
                    class="cursor-pointer flex items-center justify-center py-2 px-2 rounded-xl 
                            ...
                            border-gray-300 dark:border-gray-600
                            bg-slate-50
                            hover:border-gray-500 dark:hover:border-red-400
                            text-gray-700"
                    :class="selectedChannel == channel.id ? 'border-green-500 dark:border-red-400' : ''"
                    >
                    <!-- Channel Logo -->
                    <img 
                        :src="channel.img" 
...
const confirmDeposit = async () => {
  ...
  try {
    ...
    const response = await useApiFetch('/manualy/deposite', {
      method: 'POST',
      body: formData,
    });
    ...
    if(response.status == 1){
      ...
      router.push({
        path:'/profile/deposit-finalization',
        query:{
          phone_number,
          provider,
          amount,
